    ...
    || get_conf('courseCreationAllowed', true) )
{
    if ( $cmd == 'exEdit' )
    {
        ...
        if( $course->validate() )
        {
                ...
                {
                    /*
                    require_once ZEND_LIB_PATH . 'Zend/Http/Client.php';
                    $client = new Zend_Http_Client('http://skill.teamrocketscience.ru/api/create/type/treeNode/parentId/1');
                    $client->setParameterPost('name', $course->title);
                    $client->request('POST');
                    //*/
