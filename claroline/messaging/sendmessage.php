...
if (isset($_REQUEST['cmd']) && in_array($_REQUEST['cmd'], $acceptedCmdList) )
{
    ...
    if ($_REQUEST['cmd'] == 'rqMessageToUser' && isset($_REQUEST['userId']))
    {
        ...
        $props = user_get_properties($userId);
        if(is_array($props) && isset($props['firstname']) && isset($props['lastname']))
...
$claroline->display->body->appendContent(claro_html_tool_title(get_lang('Compose a message to ' . $to)));
