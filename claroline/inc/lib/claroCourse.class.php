<?php // $Id: claroCourse.class.php 12993 2011-03-21 13:37:53Z abourguignon $
...
class ClaroCourse
{
    ...
    protected function _initFieldRequiredList()
    {
    ...
    public function validate ()
    {
        ...
        $fieldRequiredStateList = $this->_initFieldRequiredList();
        ...
        // Se#
        foreach($fieldRequiredStateList as $field => $value)
        {
            if(empty($this->$field) && $value)
            {
                $this->backlog->failure(get_lang('Course ' . $field . ' needed'));
                $success = false ;
            }
            else
            {
                $method = '_check' . ucfirst($field);
     ...
     * @return bool
     */
    protected function _checkExtLinkUrl()
    {
     ...
     * @return bool
     */
    protected function _checkOfficialCode()
    {
     ...
     * @return bool
     */
    protected function _checkExpirationDate($value)
    {
        if ( !empty($this->expirationDate) && $value)
        {
